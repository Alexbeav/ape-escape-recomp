...
#
# Writes: dist/Ape-Escape-Recomp-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix Ape-Escape-Recomp \
  --exe-name Ape_Escape_Recompiled \
  --display-name "Ape Escape Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Ape Escape disc" \
